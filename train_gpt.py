...
def main() -> None:
    ...
    if master_process and PLOTLY_AVAILABLE:
        ...
        fig = go.Figure()
        fig.add_trace(
            go.Scatter(mode="lines", name="Train Loss", line=dict(color="blue"))
        )
        fig.add_trace(go.Scatter(mode="lines", name="Val Loss", line=dict(color="red")))
        fig.update_layout(
    ...
    def update_chart(
        ...
        fig.update_layout(
            title=f"Training and Validation Loss (Step {step})",
            xaxis_title="Step",
            yaxis_title="Loss",
            template="plotly_dark",
            autosize=True,
    ...
    while True:
        ...
        if should_log_train:
            ...
            update_chart(train_loss.item(), None, step)
